...
const Header: React.FunctionComponent = (): React.ReactElement => {
  const {isAuth, user, cart} = useSelector((state: AppState) => {return state.users})
  return (
    <div className={styles.header_container}>
        <h3>Exclusive</h3>
        <div className={styles.nav_menu_links}>
            <ul className={styles.nav_menu_list}>
                <li className={styles.nav_link}> <a href="/">Home</a></li>
                <li className={styles.nav_link}> <a href="/">Contacts</a></li>
                <li className={styles.nav_link}> <a href="/">About</a></li>
                <li className={styles.nav_link}>{user?.username}</li>
                {isAuth? <li className={styles.nav_link}>Logout</li> : <li className={styles.nav_link}> <Link to={'/register'}> Sing Up</Link></li>}
            </ul>
        </div>
        <div className={styles.header_func}>
            <p> <Link className={styles.cart_icon} to={'/cart'}>
              <p className={styles.cart_length}>{cart?.length}</p>
